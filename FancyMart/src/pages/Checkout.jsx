...
// ── Theme colours (matches theme.js)
const THEME       = '#6b21a8';
const THEME_HOVER = '#4c1d95';
const THEME_LIGHT = 'rgba(107,33,168,0.06)';
...
const RoundRadio = ({ checked, onChange }) => (
  ...
  >
    {checked && <Box sx={{ width: 10, height: 10, borderRadius: '50%', bgcolor: THEME }} />}
  </Box>
);
...
const PaymentRow = ({ value, selected, onSelect, icon, label, badges, noBorderBottom, children }) => (
  <Box
    sx={{ borderBottom: noBorderBottom ? 'none' : '1px solid #eee', cursor: 'pointer', '&:hover': { bgcolor: selected ? 'transparent' : '#fafafa' } }}
    onClick={() => onSelect(value)}
  >
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, px: 2, py: 1.5, minHeight: 56, flexWrap: 'wrap' }}>
      <RoundRadio checked={selected} onChange={() => onSelect(value)} />
      {icon && <Box sx={{ color: '#666', display: 'flex', alignItems: 'center' }}>{icon}</Box>}
      <Typography sx={{ fontWeight: 500, fontSize: '0.88rem', color: '#333', flex: 1, minWidth: 80 }}>{label}</Typography>
      {badges && (
        <Box sx={{ display: { xs: 'none', sm: 'flex' }, gap: 0.75, alignItems: 'center' }}>
          {badges.map((b, i) => (
            <Box key={i} sx={{ border: '1px solid #ddd', borderRadius: '3px', px: 0.8, py: 0.3, display: 'flex', alignItems: 'center', justifyContent: 'center', minWidth: 44, height: 26, bgcolor: 'white' }}>
              {b}
            </Box>
          ))}
        </Box>
...
// ── Form Field ────────────────────────────────────────────────────────────────
const FField = ({ label, error, helperText, ...rest }) => (
  <Box>
    {label && (
    ...
    <TextField
      fullWidth size="small" error={error} helperText={helperText} {...rest}
      sx={{
        mb: 0,
        '& .MuiOutlinedInput-root': {
          borderRadius: '3px', fontSize: '0.82rem', bgcolor: '#fff',
...
const Checkout = () => {
  ...
  return (
    <Box sx={{ bgcolor: '#f5f5f5', minHeight: '100vh' }}>
      ...
      {/* ── Banner ── */}
      <Box sx={{ width: '100%', bgcolor: THEME, py: { xs: 1.8, md: 2.2 }, textAlign: 'center', mb: { xs: 2.5, md: 4 } }}>
        <Typography
          variant="h5"
          sx={{ color: 'white', fontWeight: 700, fontSize: { xs: '1.1rem', sm: '1.3rem', md: '1.5rem' } }}
        >
          Checkout{' '}
          <Typography component="span" sx={{ color: 'rgba(255,255,255,0.82)', fontWeight: 400, fontSize: 'inherit' }}>
            ({String(itemCount).padStart(2, '0')} items)
          </Typography>
        </Typography>
      </Box>

      <Container maxWidth="lg" sx={{ pb: 8, px: { xs: 1.5, sm: 2, md: 5 } }}>
        {/*
          Responsive layout:
          ─ Mobile (xs):  single column — Confirm box stacks ABOVE Pay with
          ─ Tablet+ (sm): flex row — Left (Pay with + Ship to) | Right (Confirm box)
        */}
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', sm: 'row' }, gap: { xs: 2, sm: 3 }, alignItems: 'flex-start' }}>

          {/* ═══ LEFT COLUMN ═══ */}
          <Box sx={{ flex: '1 1 0%', minWidth: 0, order: { xs: 2, sm: 1 } }}>

            {/* ── PAY WITH ── */}
            <Box sx={{ bgcolor: '#fff', border: BORDER, borderRadius: '5px', mb: 2.5, overflow: 'hidden' }}>
              <Box sx={{ px: 2, pt: 1.8, pb: 1.2 }}>
                <Typography sx={{ fontWeight: 700, fontSize: '0.9rem', color: '#222' }}>Pay with</Typography>
              </Box>
              <Divider />

              {/* Card */}
              <PaymentRow
                value="card" selected={paymentMethod === 'card'} onSelect={setPaymentMethod}
                label="Add new card" badges={[<VisaLogo />, <MCLogo />, <AmexLogo />]}
              >
              ...
              {/* Bank Transfer */}
              <PaymentRow
                value="bank" selected={paymentMethod === 'bank'} onSelect={setPaymentMethod}
                icon={<BankIcon sx={{ fontSize: 20 }} />} label="Bank Transfer"
              >
                <Box sx={{ bgcolor: '#f7f7ff', border: '1px solid #e8e8f0', borderRadius: '4px', p: 1.2, mb: 1.2 }}>
                  <Typography variant="caption" color="text.secondary">
                    Bank: <strong>Commercial Bank</strong> &nbsp;|&nbsp; Acc: <strong>8001-2345-6789</strong>
                  </Typography>
                </Box>
                <Box component="label" htmlFor="slip-upload" sx={{
                  display: 'flex', alignItems: 'center', gap: 1, p: 1.2,
                  border: `1px dashed ${errors.slip ? '#d32f2f' : slipFile ? '#4caf50' : '#ccc'}`,
                  borderRadius: '4px', cursor: 'pointer',
                  '&:hover': { borderColor: THEME, bgcolor: THEME_LIGHT },
                }}>
                  <input id="slip-upload" type="file" hidden accept="image/*,.pdf"
                    onChange={(e) => {
                      if (e.target.files?.[0]) { setSlipFile(e.target.files[0]); setErrors((p) => ({ ...p, slip: '' })); }
                    }} />
                  {slipFile ? <AttachFileIcon sx={{ color: '#4caf50', fontSize: 18 }} /> : <UploadIcon sx={{ color: '#999', fontSize: 18 }} />}
                  <Typography variant="body2" sx={{ color: slipFile ? '#4caf50' : '#888', fontSize: '0.8rem', fontWeight: slipFile ? 600 : 400, wordBreak: 'break-all' }}>
                    {slipFile ? slipFile.name : 'Upload payment slip (JPG / PNG / PDF)'}
                  </Typography>
                </Box>
                {errors.slip && <Typography variant="caption" sx={{ color: '#d32f2f', mt: 0.5, display: 'block' }}>{errors.slip}</Typography>}
              </PaymentRow>

              {/* Cash on Delivery */}
              <PaymentRow
                value="cod" selected={paymentMethod === 'cod'} onSelect={setPaymentMethod}
                icon={<ShippingIcon sx={{ fontSize: 20 }} />} label="Cash on Delivery" noBorderBottom
              >
            ...
            {/* ── SHIP TO ── */}
            <Box sx={{ bgcolor: '#fff', border: BORDER, borderRadius: '5px', p: { xs: 1.5, sm: 1.8, md: 2.2 } }}>
              <Typography sx={{ fontWeight: 700, fontSize: '0.9rem', color: '#222', mb: 2 }}>Ship to</Typography>

              {/*
                CSS grid: 2 columns on sm+, 1 column on xs.
                MUI's sx supports responsive gridTemplateColumns values.
              */}
              <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: { xs: '12px', sm: '10px', md: '14px' } }}>
                ...
                {/* Row 3 — Shipping Address | Nearest City */}
                {/* Shipping Address — label stacks above link to avoid overlap on narrow columns */}
                <Box sx={{ minWidth: 0 }}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-start', mb: 0.4, gap: 0.2 }}>
                    <Typography variant="caption" sx={{ fontWeight: 500, color: '#555', whiteSpace: 'nowrap' }}>Shipping Address</Typography>
                    <Typography
                      variant="caption"
                      onClick={() => setShip((p) => ({ ...p, saveAsDefault: !p.saveAsDefault }))}
                      sx={{ color: THEME, fontSize: '0.65rem', textDecoration: 'underline', cursor: 'pointer', fontWeight: ship.saveAsDefault ? 700 : 400, whiteSpace: 'nowrap' }}
                    >
                ...
                <Box sx={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'flex-end', gap: 1.5, pb: '2px' }}>
                  <Button
                    variant="outlined" onClick={() => navigate('/cart')}
                    sx={{
                      width: { xs: '100%', sm: '155px' }, height: '30px',
                      borderColor: THEME, color: THEME, borderRadius: '2px', borderWidth: '2px',
                      fontSize: '0.78rem', fontWeight: 700, textTransform: 'uppercase',
                      '&:hover': { bgcolor: THEME_LIGHT, borderColor: THEME, borderWidth: '2px' },
                    }}
                  >
                    CANCEL
                  </Button>
                  <Button
                    ...
                    sx={{
                      width: { xs: '100%', sm: '155px' }, height: '30px',
                      bgcolor: THEME, color: 'white', borderRadius: '2px',
                      fontSize: '0.78rem', fontWeight: 700, textTransform: 'uppercase',
                      boxShadow: 'none', '&:hover': { bgcolor: THEME_HOVER, boxShadow: 'none' },
                    }}
                  >
                    SAVE
                  </Button>
          ...
          {/* ═══ RIGHT COLUMN — Confirm box ═══ */}
          <Box sx={{
            flexShrink: 0,
            width: { xs: '100%', sm: '220px', md: '280px', lg: '300px' },
            position: { sm: 'sticky' },
            top: { sm: 90 },
            order: { xs: 1, sm: 2 },
          }}>
            <Box sx={{ bgcolor: '#fff', border: BORDER, borderRadius: '5px', p: 2.2 }}>
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2.2 }}>
                <Typography sx={{ fontWeight: 700, fontSize: '0.92rem', color: '#222' }}>Order total</Typography>
              ...
              <Button
                ...
                sx={{
                  bgcolor: THEME, color: 'white', py: 1.3, borderRadius: '4px',
                  fontWeight: 700, textTransform: 'none', fontSize: '0.88rem', boxShadow: 'none',
                  '&:hover': { bgcolor: THEME_HOVER, boxShadow: `0 2px 8px rgba(107,33,168,0.35)` },
                }}
              >
                Confirm and pay
              </Button>
